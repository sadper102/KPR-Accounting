      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 pb-10 border-b border-slate-200">
            {accreditations.map((acc, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center gap-3.5 p-4 rounded-xl bg-slate-50 border border-slate-200 shadow-sm"
                >
                  <div className="w-10 h-10 rounded-lg bg-[#0D2240] text-[#C5A059] flex items-center justify-center shrink-0">
                    <IconComp className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="text-[#0D2240] font-extrabold text-xs sm:text-sm">
                      {acc.title}
                    </div>
        ...
        <ScrollReveal direction="up" delay={0.2}>
          <div className="pt-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-xs font-black text-[#0D2240] uppercase tracking-wider shrink-0">
              ประเภทธุรกิจที่ไว้วางใจ KPR ACCOUNTING:
            </div>
            
            <div className="flex flex-wrap justify-center md:justify-end gap-2.5 sm:gap-4 text-xs font-bold text-slate-800">
              {clientSectors.map((sector, sIdx) => (
                <span
                  key={sIdx}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 border border-slate-300"
                >
                  <CheckCircle2 className="w-3.5 h-3.5 text-[#9A7B39]" />
                  <span>{sector}</span>
                </span>
              ))}
            </div>
